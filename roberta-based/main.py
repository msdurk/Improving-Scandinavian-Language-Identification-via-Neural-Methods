...
def add_language2id(dataset):
    """ Function to generate column 'label' (for language2id mapping)
    Args:
        dataset (dataset loaded with huggingface load_dataset)
    Returns:
        dataset (with mappings)
        lang2id (dictionary with the mapping from language to id)
    """
...
model = XLMRobertaForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=len(lang2id))
model.to(device)
